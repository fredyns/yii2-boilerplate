...
/**
 * Description of RgnProvinceControl
 * manage access control and operation available in frontend
 *
 * @author fredy
 */
class RgnProvinceControl extends ModelControl
{
	...
	public function getIsActionDeleteAllowed()
	{
		...
		if (array_key_exists('delete', $this->actionAllowed) == FALSE)
		{
			...
			// action whitelist
			else if ($this->model->status === RgnProvince::STATUS_ACTIVE)
			{
			...
			{
				if ($this->model->status === RgnProvince::STATUS_DELETED)
				{
	...
	public function getIsActionRestoreAllowed()
	{
		...
		if (array_key_exists('restore', $this->actionAllowed) == FALSE)
		{
			...
			// action whitelist
			else if ($this->model->status === RgnProvince::STATUS_DELETED)
			{
			...
			{
				if ($this->model->status === RgnProvince::STATUS_ACTIVE)
				{
					$this->setActionError('restore', 'Data already active.');
				}
				else if ($this->model->status === RgnProvince::STATUS_ARCHIVE)
				{
